...
class MatchGame extends Component {
  state = {
    score: 0,
    timer: 60,
    bannerImgUrl: imagesList[0].imageUrl,
    bannerImgId: imagesList[0].id,
  ...
  renderLandingPage = () => {
    const {bannerImgUrl, activeTabId} = this.state

    return (
      <div className="landing-page-container">
        <img src={bannerImgUrl} alt="match" className="banner-image" />
  ...
  validateThumbnail = ThumbnailId => {
    const {bannerImgId} = this.state
    const isValid = bannerImgId === ThumbnailId
    const bannerImgIndex = Math.floor(Math.random() * imagesList.length)
    console.log(bannerImgIndex)
    if (isValid) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        bannerImgUrl: imagesList[bannerImgIndex].imageUrl,
        bannerImgId: imagesList[bannerImgIndex].id,
      }))
    } else {
      this.setState({
        timer: 0,
            ...
            <ul className="thumbnail-container">
              {filteredList.map(eachItem => (
                <ThumbnailItem
                  ...
                  key={eachItem.id}
                  validateThumbnail={this.validateThumbnail}
                />
              ))}
            </ul>
          </>
